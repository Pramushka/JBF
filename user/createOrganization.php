...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "INSERT INTO organization (UserID, Org_Name, Org_Email, Org_Register_no, Org_Location, Org_Industry, Verification_Contact) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($query);
    $userId = $_SESSION['user_id'];  // Assuming user's ID is stored in session
    $stmt->bind_param("issssss", $userId, $orgName, $orgEmail, $orgRegisterNo, $orgLocation, $orgIndustry, $verificationContact);
